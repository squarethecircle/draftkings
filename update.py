...
def calc_ppg(final_frame):
    ...
    for index,row in sort_frame.iterrows():
        ...
        last_pts = sort_frame.loc[index,'Total Points'] + sort_frame.loc[index,'Points']
...
def grab_history(week,year):
    url = 'http://www.rotoguru1.com/cgi-bin/fyday.pl?week=%s&year=%s&game=%s&scsv=1' % (week, year, RT_YEAR[year])
    print url
    page = safe_get(url)
    start_anchor = '<pre>'
    end_anchor = '</pre>'
...
def grab_projections(week, year, pos):
    if year < CUR_YEAR:
        url = 'http://www.fantasypros.com/nfl/rankings/%s.php?week=%s&year=%s&export=xls' % (pos, week, year)
    else:
        url = 'http://www.fantasypros.com/nfl/rankings/%s.php?week=%s&export=xls' % (pos, week)
    print url
    return safe_get(url).replace('\t\t\n','\n')


def new_history_dataframe(raw_data):
    new_frame = pd.read_csv(StringIO(raw_data), sep=";", dtype=np.dtype(str),names=['Week','Year','GID','Name','Pos','Team','h/a','Oppt','Points','Salary'],skiprows=1)
...
if __name__ == '__main__':
    ...
    hist_frame = pd.concat([hist_frame,recent_results], ignore_index = True).drop_duplicates(['PID','Year','Week'],keep='last')
    cur_projs = []
    for pos in [FP_QB,FP_FLEX,FP_DST]:
    ...
    final_frame = pd.concat([hist_frame,cur_frame], ignore_index = True).drop_duplicates(['PID','Year','Week'],keep='last')
